...
class FusionLayer(nn.Module):
    def __init__(
        ...
        # Per-frame feature extraction (shared weights across all frames)
        self.feature_extractor = FeatureExtractor(in_channels, num_features)

        # Reduce concatenated features [prv, cur, nxt] → num_features
    ...
    def forward(self, upsampled_base, prv, nxt):
        f_prv = self.feature_extractor(prv)
        f_cur = self.feature_extractor(upsampled_base)
        f_nxt = self.feature_extractor(nxt)

        # Temporal fusion via concatenation
        fused = torch.cat([f_prv, f_cur, f_nxt], dim=1)
